...
impl Region {
    ...
    fn perimeter(&self, field_labels: &Vec<Vec<usize>>) -> u32 {
        (0..self.coordinates.len())
            .map(|i| self.cell_perimeter(i, field_labels))
            .fold(0, |a, b| a + b)
    }

    fn cell_perimeter(&self, idx: usize, field_labels: &Vec<Vec<usize>>) -> u32 {
        let coord = self.coordinates[idx];
        let mut perimeter = 0;
        let directions = [(0, 1), (1, 0), (0, -1), (-1, 0)];

        for (dx, dy) in directions.iter() {
            let new_x = coord.0 as isize + dx;
            let new_y = coord.1 as isize + dy;
            ...
            {
                perimeter += 1;
            } else if field_labels[new_x as usize][new_y as usize] != self.label {
                perimeter += 1;
            }
        }
